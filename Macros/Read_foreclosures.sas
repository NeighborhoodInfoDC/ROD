/**************************************************************************
 ...
 Modifications:
  ...
  09/04/12 PT  Added new geographies, Parcel_geo_match var.
               Added %local declaration for local macro vars.
               New macro var geo_list includes all geo vars to be saved, 
               except x_coord, y_coord which are only in Parcel_geo.
...
%macro Read_foreclosures( files=, year=, month=, finalize=N, revisions=New file. );
  ...
  %local out out_nolib geo_list i f;

  %let finalize = %upcase( &finalize );

  %if &finalize = Y %then %do;
    %note_mput( macro=Read_foreclosures, msg=Finalize=&finalize - Rod_R.Foreclosures_&year will be replaced. )
    %let out = Rod_R.Foreclosures_&year;
    %let out_nolib = Foreclosures_&year;
  %end;
  %else %do;
    %warn_mput( macro=Read_foreclosures, msg=Finalize=&finalize - Rod_R.Foreclosures_&year will NOT be replaced. )
  ...
  ** Print documents that did match with square file but not with parcel file**;
    ...
    proc print data=&out n='Total to check = ';
      where missing( X_COORD ) and not( missing( ward2012 )) and missing ( update_checked );
      id FilingDate DocumentNo;
      var Instrument ssl square lot xlot ward2012 MultipleLots Verified update_checked;
  ...
  title2;
    ...
    ** Print documents that are co-ops to check notice for unit or building**;
      ...
      proc print data=&out n='Total to check = ';
        ...
        id FilingDate DocumentNo;
        var Instrument ssl ui_proptype ward2012 MultipleLots Verified update_coop ;
  ...
  ** Basic file info **;
  ...
  %File_info( data=&out, printobs=5, freqvars=Verified Instrument UI_Instrument BookType MultipleLots ui_proptype Parcel_geo_match ward2012)
