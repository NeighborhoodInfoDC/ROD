/**************************************************************************
 ...
 Modifications: 02/06/12 LH Updated for 2011
  ...
  NOTE: NEED TO RUN A TEST TO COMPARE SUMMARIES IF WE USE EXISTING GEOS
  IN DISAGGREGATED FORECLOSURE FILES.
**************************************************************************/
...
%macro Foreclosure_sum_geo( geo=, start_yr=, end_yr=, revisions=, register=N);
  ...
  ** DENOMINATOR INFO = # of Parcels for geo unit & year **;
	  ...
	  proc transpose data=realprop.num_units&geosuf out=sf;
		by &geo;
		var units_sf_1995-units_sf_2011;
		run;
	  ...
	  proc transpose data=realprop.num_units&geosuf out=condo;
		by &geo;
		var units_condo_1995-units_condo_2011;
		run;
	  ...
	  proc transpose data=realprop.num_units&geosuf out=sfcondo;
		by &geo;
		var units_sf_condo_1995-units_sf_condo_2011;
		run;
  ...
  ** Merge Geo Sum Files **;
  ...
  proc compare base=Save.Foreclosures_sum&geosuf compare=Rod.Foreclosures_sum&geosuf maxprint=(40,32000)
      method=percent criterion=1;
    id &geo;
  run;
...
libname save "D:\DCData\Libraries\ROD\Data\Save";
