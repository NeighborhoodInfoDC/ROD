...
%macro Foreclosure_sum_geo( geo=, start_yr=, end_yr=, revisions=, register=N);
  ...
  %local sum_vars sum_vars_ssl sum_flag_ssl sum_vars_rates sum_vars_wf sum_vars_wt
   ...
   %if &register = Y %then %do;
    %note_mput( macro=Foreclosure_sum_geo, msg=Register=&register - ROD_r.Foreclosures_sum&geosuf will be replaced. )
    %let out = ROD_r.Foreclosures_sum&geosuf;
    %let out_nolib = Foreclosures_sum&geosuf;
  %end;
  %else %do;
    %warn_mput( macro=Foreclosure_sum_geo, msg=Register=&register - ROD_r.Foreclosures_sum&geosuf will NOT be replaced. )
    %let out = Foreclosures_sum&geosuf;
    %let out_nolib = Foreclosures_sum&geosuf;
  %end;
  ...
  ** SUMMARIZE FOR PARCEL LEVEL NOTICE INFO **;
  ...
  proc sql;
       ...
       select parcels_not.*, geo.*
       	from parcels_not left join RealPr_r.Parcel_geo (drop=CJRTRACTBL) 
  ...
  ** DENOMINATOR INFO = # of Parcels for geo unit & year **;
	  ...
	  proc transpose data=RealPr_r.num_units&geosuf out=sf;
		by &geo;
	  ...
	  proc transpose data=RealPr_r.num_units&geosuf out=condo;
		by &geo;
	  ...
	  proc transpose data=RealPr_r.num_units&geosuf out=sfcondo;
		by &geo;
