/**************************************************************************
 Program:  Foreclosures_reo_history.sas
 Library:  Rod
 ...
 Description:  Create reo history file with records for individual property reo episodes.
 	       Based on foreclosures_history.sas
...
if first.ssl then do; 
	...
	firstcancel_date=.; lastcancel_date=.; 	num_notice=0 ; num_cancel=0; num_sales=0;
...
if end=1 and record_type="Foreclosure Sale/REO" and next_record_type ne "Trustees Deed" and prev_sale_owncat in("120" "130")
and post_sale_accept="05" and num_notice=0 then do; record_type="Other Sale"; outcome_code=.n; outcome_date=.n; post_sale_reo=1; end;*/
...
if post_sale_reo=0 then do;
	...
	if prev_sale_owncat in ('010' '020' '030' '110' '115') and post_sale_owncat in ('040' '050' '120' '130') and tdeed_date= . then do;
															...
															outcome_code=3; 
															outcome_date=filingdate; post_sale_reo=1; 			
	end;
*assigning reo for commerically owned properties;															
 if ui_proptype in ("10" "11") and (post_sale_accept='05' or num_notice >0)then do;
	if prev_sale_owncat in ('010' '020' '030' ) and post_sale_owncat in ('115') and tdeed_date ne . then do;
													record_type="TD/Foreclosure Sale/REO";
													outcome_code=2; 
												    outcome_date=tdeed_date; 
													post_sale_reo=1;
	...
	if prev_sale_owncat in ('010' '020' '030' ) and post_sale_owncat in ('115') and tdeed_date= . then do;
												    record_type="Foreclosure Sale/REO"; 
													outcome_code=3;
...
if prev_record_type ="Foreclosure Sale/REO" and record_type = "Trustees Deed" and (0 <= prev_record_days <=31) then do;
 ...
 /*dropping observations with no foreclosure notice/sale in owner/property episode; *need to insert prgm code to make reo history file.
...
keep ssl filingdate_R filingdate ui_instrument num_notice  year firstnotice_date lastnotice_date lastnotice_grantee lastnotice_grantor
	...
	record_type prev_record_type next_record_type end prev_record_days order lastnotice_xlot lastnotice_multiplelots ui_proptype;
run;
%mend;
%order2;
...
proc print data=step4 (obs=1000);
where post_sale_reo=1;
var ssl filingdate record_type outcome_code prev_sale_owner prev_sale_owncat post_sale_owner num_notice ui_proptype;
run;
...
data rod.&out (label="Foreclosure history, DC");
	set step10;
drop end lag_sale_num lag2_sale_num num_sales;
