/**************************************************************************
 Program:  Monthly_foreclosures_2009_09.sas
 Library:  ROD
...
%let start_date = '01sep2007'd;
%let end_date = '30sep2009'd;
%let suffix = 2009_09;
...
proc download status=no
  ...
  select Notices_monthly_ssl Notices_monthly;
...
data Rod.Notices_monthly_ssl_&suffix (compress=no);
  set Notices_monthly_ssl;
  *where year( filingdate ) = year( &end_date );
  where yymm =: "2009";
run;

proc freq data=Rod.Notices_monthly_ssl_&suffix;
  tables ui_proptype * ui_instrument / list nocum nopercent;
  title2 'Notice counts for maps';
run;
