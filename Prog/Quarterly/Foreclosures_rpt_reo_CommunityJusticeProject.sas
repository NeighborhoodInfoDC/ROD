 /**************************************************************************
 Program:  Foreclosures_rtp_reo_CommunityJusticeProject.sas
 Library:  HsngMon
...
%let suffix=_CJP;
%let end_date='31mar2012'd;
%let qtr=2012Q1;

/* New foreclosures_history file should be downloaded after each quarterly update*/
/*
rsubmit;
...
%macro Foreclosures_reo( out_ds=, unit=, rpt_start_dt='01jan1999'd, rpt_end_dt= );
  ...
  data &out_ds (where = (in_reo_end = 1));
    ...
    ** Create obs. for each ssl/episode/qtr with outcome vars. **;
    ...
    keep 
      ssl city ward2012 /*ward2012*/ zip post_sale_owncat post_sale_ownerr report_dt end_dt adj_end_dt outcome_date outcome_code2 
...
data Forecl_reo (where = (in_reo_end = 1 and report_dt = '01jan2012'd) keep = premiseadd ssl ward2012 zip post_sale_owncat post_sale_ownerr report_dt end_dt adj_end_dt outcome_date outcome_code2 
	  in_reo_end reo_start post_sale_reo firstnotice_date);
...
by ward2012 premiseadd;
run;

ods html file="K:\Metro\PTatian\DCData\Libraries\ROD\Prog\Quarterly\REO_&qtr._&suffix..xls" style=minimal;

proc print data=Forecl_reo label noobs;
  var Premiseadd ward2012 post_sale_ownerr firstnotice_date outcome_date;
  label 
	Premiseadd = "Address"
	Zip = "ZIP"
	ward2012 = "2012 Ward"
