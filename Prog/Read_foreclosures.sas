  ...
  ** Merge with geographies **;
  ...
  proc sql;
    ...
    select all_files_fix2.*, geo.*, base.ssl, base.ui_proptype
    	from all_files_fix2 left join RealProp.Parcel_geo (keep=ssl x_coord y_coord &geo_list) as geo on (all_files_fix2.ssl = geo.ssl)
  ...
  ** Merge in Square geo **;
  proc sql;
      ...
      select *
      	...
      	left join RealProp.Square_geo (keep=square &geo_list) as geo
