...
merge who_owns (in=a  rename=(saledate=filingdate))
      ...
      tdeed (in=b keep=ui_instrument ssl filingdate grantee grantor xlot multiplelots ui_proptype ownercat
...
length record_type $23. prev_record_grantee prev_record_grantor $80. prev_record_accept prev_record_hstd $1.
...
if prev_record_type not in ("Notice" "Cancellation" "Trustees Deed") then do;
...
if first.ssl then do;
    num_notice=0 ; num_cancel=0; num_sales=0; num_tdeed=0; prev_rule=.;  post_rule=.; firstnotice_date=. ;
    firstcancel_date=.; outcome_date=.; post_sale_reo=.;
    %do i=1 %to 22; %let num=%scan(&numericlist.,&i.," "); &num.=.; %end;
    %do j=1 %to 46; %let char=%scan(&characterlist.,&j.," "); &char.=" "; %end;
end;
...
*/
if end=. and record_type="Notice" and prev_record_type in ("Other Sale" "Foreclosure Sale/REO" "TD/Foreclosure Sale/REO"
...
if end=1 and lastcancel_date=. and firstcancel_date ne . and prev2_record_type="Cancellation" and prev_record_type ne "Cancellation"
    and record_type not in("Cancellation") then do;
        %do k=1 %to 8; %let lastcancel=%scan(&lastcancellist.,&k.," "); %let prevrecrod=%scan(&prevrecRODlist.,&k.," ");
                            &lastcancel.=&prevrecrod.; %end;
                                            end;
...
if end=1 and record_type in ("Other Sale" "Pre 1998 Sale") and num_notice = 0 then do; outcome_code=.n; outcome_date=.n; end;
...
those still in the foreclosure inventory;
...
if prev_sale_num = . and record_type in("Notice" "Cancellation") and lag_sale_num ne . and order ne 1 then do;
...
if prev_sale_num = . and record_type  in("Notice" "Cancellation") and lag_sale_num = . and lag2_sale_num ne . and order not in(1 2) then do;
...
if prev_sale_num = . and record_type in("Notice" "Cancellation") and lag_sale_num = . and lag2_sale_num = . and lag3_sale_num ne .
...
if post_sale_owncat ne " " and outcome_code not in(1 5 6) and post_sale_reo=. then post_sale_reo=0;
...
**finish reo;
if outcome_code in (2) and post_sale_owncat in ('120' '130') then post_Sale_reo=1;
if outcome_code in (2 3) and post_sale_owncat=" " and post_sale_num ne . and post_sale_reo=. then post_sale_reo=.U;
if outcome_code in (1 4 5 6) then post_sale_reo=.;
...
keep ssl filingdate_R filingdate ui_instrument num_notice  year firstnotice_date &lastnoticelist. &firsttdeedlist. &lasttdeedlist. &lastcancellist.
    ...
    &prevsalelist. &postsalelist. record_type prev_record_type next_record_type end prev_record_days order;
run;
...
title2 "step4a";
tables outcome_code order post_sale_reo post_sale_reo*record_type post_sale_Num prev_sale_prp/missprint;
format outcome_code outcome.  ;
run;
...
merge step8 (in=a drop=lastnotice_granteeR firsttdeed_lastname lasttdeed_lastname lastcancel_granteeR lastcancel_lastname
					   lastnotice_owncat lastcancel_owncat ownername ownname2)
