/**************************************************************************
 ...
 Description: Create foreclosure history file with records for  individual property foreclosure episodes.

 Modifications:  

07/09/09 LH - Updated with Sales data through 3/31/09 and Foreclosures through 6/30/09 and expanded number available
...
02/26/14 LH	- Updated for SAS1. Incorporated metadata macro into this program instead of Register_foreclosures_history.sas. 
			  Updated Sales through 08/13/13, Forecl through 06/30/13.	
08/11/14LH -  Updated Sales through 12/27/13, Forecl through 12/31/13.	
**************************************************************************/
...
/***ARCHIVE OLD DATA SET BEFORE RUNNING***/ 

%let data       = sales_master;
*%let out        = Foreclosures_history; *now (2/26/14) below in %finalstep macro.;
%let RegExpFile = Owner type codes & reg expr 09-28-11.xls/*Owner type codes & reg expr LH.xls*/;
%let MaxExp     = 1000;
%let start_dt = '01jan1990'd;
%let end_dt   = '31dec2013'd;
%let register = Y;
%let revisions = %str(Updated Sales through 12/27/13, Forecl through 12/31/13.);
%let hist_label =through 12/31/13.; 
...
by ssl;
proc sort data=realpr_r.parcel_base out=parcel_base;
by ssl;
proc sort data=realpr_r.parcel_geo out=parcel_geo;
by ssl;
data step9;
...
%macro finalstep;
   ...
   %if &register = Y %then %do;
    %note_mput( macro=finalstep, msg=Register=&register - ROD.Foreclosures_history will be replaced. )
    %let out = ROD.Foreclosures_history;
    %let out_nolib = Foreclosures_history;
  %end;
  %else %do;
    %warn_mput( macro=finalstep, msg=Register=&register - ROD.Foreclosures_history will NOT be replaced. )
    %let out = Foreclosures_history;
    %let out_nolib = Foreclosures_history;
  %end;

data &out (label="Foreclosure history, DC &hist_label" drop=end lag_sale_num lag2_sale_num num_sales sortedby=ssl order);
    set step10;
